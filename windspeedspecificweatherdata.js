...
const top25WindSpeed = (timeInterval) => {
    ...
    const URI =
      timeInterval === "now"
        ? weatherdata30
        : "24"
        ? `${direcHrAvgUri}/wind_speed/23`
        : "48"
        ? `${direcHrAvgUri}/wind_speed/47`
        : "72"
        ? `${direcHrAvgUri}/wind_speed/71`
        : `${direcHrAvgUri}/wind_speed/167`;
    ...
    fetch(URI)
      ...
      .then((data) => {
        ...
        data.map((data, index) => {
          ...
          cell4.innerHTML = data.data[`wind_speed`];
        });
